...
// Sleep capability:
// Change sleep and jitter values on ant prorcess
...
// Get current ant's process working directory
type CapPwd struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
...
// Change process working directory of ant's process
type CapCd struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Path to directory
	Path          string `protobuf:"bytes,1,opt,name=path,proto3" json:"path,omitempty"`
...
// Whoami capability:
// Get information about user under which ant's process working
...
// Jobs capability:
// Get active jobs (other capabilities execution flows) on ant
type CapJobs struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
...
// Kill job capability:
// Kill active job on ant specified by its ID
...
// Execute .NET binary with arguments in CLR runtime in ant's process memory
...
// Pause capability:
// Pause ant's process execution on specified value (in seconds)
type CapPause struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Number of seconds to pause ant's execution
...
// Destruct ant itself (with removing of executable and self killing)
...
// Spoof PPID on ant's process on specified value. For rollback you can implement logic of special value, e.g. 0
type CapPpid struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Value to spood PPID on. In case of rollback can be used special value as 0 (must be handled by ant)
...
var file_common_v1_common_proto_rawDesc = []byte{
	...
	0x20, 0x01, 0x28, 0x04, 0x52, 0x04, 0x70, 0x70, 0x69, 0x64, 0x22, 0x09, 0x0a, 0x07, 0x43, 0x61,
	0x70, 0x45, 0x78, 0x69, 0x74, 0x42, 0x3f, 0x5a, 0x3d, 0x67, 0x69, 0x74, 0x68, 0x75, 0x62, 0x2e,
	0x63, 0x6f, 0x6d, 0x2f, 0x50, 0x69, 0x63, 0x6f, 0x54, 0x6f, 0x6f, 0x6c, 0x73, 0x2f, 0x70, 0x69,
	0x63, 0x6f, 0x2d, 0x73, 0x68, 0x61, 0x72, 0x65, 0x64, 0x2f, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x2f,
